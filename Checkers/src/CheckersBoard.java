...
class CheckersBoard
{
    ...
    //******************************************************
    // CheckersBoard constructor used to put pieces on board.
    //******************************************************
    ...
    {
        if (board[x][y].getColor().charAt(1) == 'K')
            return true;

        if (board[x][y].getColor().equals("  "))
            return false;
        
        if (Main.turnColor.equals("WT"))
            if ((x < 5) && board[x][y].getColor().charAt(0) == 'W')
                return true;
        
        if (Main.turnColor.equals("RD"))
            if ((x > 2) && board[x][y].getColor().charAt(0) == 'R')
                return true;
    ...
    public boolean validMove(String from, String to) //unfinished
    {
        ...
        if (Math.abs(num3 - num) > 1 || Math.abs(num4 - num2) > 1)
        {
            ...
            {   
                ...
                if (board[num6][num5].getColor().charAt(0) != Main.turnColor.charAt(0) && !board[num6][num5].getColor().equals("  "))
                {
    ...
    public boolean jumpAvailable(String startPos)
    {
        ...
        // White jumps
        if (Main.turnColor.equals("WT") || board[num2][num].getColor().charAt(1) == 'K')
        {
            // minus minus
            // if num-2 and num2-2 is a blank space, num-1 and num2-1 are NOT the same color or blank.
            if (num2 - 2 > -1 && num - 2 > -1)
            {
                if (board[num2-2][num-2].getColor().equals("  ") && !(board[num2-1][num-1].getColor().equals(Main.turnColor) || (board[num2-1][num-1].getColor().equals("  "))))
                {
        ...
        // Red jumps
        if (Main.turnColor.equals("RD") || board[num2][num].getColor().charAt(1) == 'K')
        {
            // plus minus
            if (num2 + 2 < 8 && num - 2 > -1) 
            {
            ...
            {   
                ...
                for(int i = 0; i < 8; i++)
                {
                    for (int j = 0; j < 8; j++)
                    {
                        if(i == num6 && j == num5)
                        {
                            if(!(board[i][j].getColor().equals(Main.turnColor)))
                            {     
                                board[i][j].setColor("  ");
                            }
                        }
		...
		for(int i = 0; i < 8; i++)
		{
			for (int j = 0; j < 8; j++)
			{
				if(board[i][j].getColor().equals("RD"))
					...
					whiteCheckers++;
				else if (board[i][j].getColor().equals("RK"))
					redCheckers++;
				else if (board[i][j].getColor().equals("WK"))
					whiteCheckers++;
			}
		}
